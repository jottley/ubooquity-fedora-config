...
case "$1" in
    start)
        if [ -z "$count" ];
        then
            nohup java $MEM_OPT -jar /srv/ubooquity/Ubooquity.jar --headless --workdir /var/local/ubooquity --libraryport $UBOOQUITY_PORT --adminport $ADMIN_PORT --remoteadmin </dev/null &>/var/log/ubooquity.log &
